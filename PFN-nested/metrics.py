...
class macro():
    ...
    def count_num(self, ner_pred, ner_label, re_pred_head, re_pred_tail, re_label_head, re_label_tail):
        ...
        for r in range(len(self.rel2idx)):
            ...
            re_pred_head_single = re_pred_head[:, :, :, r]
            re_pred_tail_single = re_pred_tail[:, :, :, r]
            re_label_head_single = re_label_head[:, :, :, r]
            re_label_tail_single = re_label_tail[:, :, :, r]
            ...
            for i in range(batch):
                ...
                re_label_head_batch = re_label_head_single[:,:,i]
                re_label_tail_batch = re_label_tail_single[:,:,i]
                re_label_set = self.get_trip(ner_label_batch, re_label_head_batch, re_label_tail_batch)

                re_pred_head_batch = re_pred_head_single[:,:,i]
                re_pred_tail_batch = re_pred_tail_single[:,:,i]
                re_pred_set = self.get_trip(ner_pred_batch, re_pred_head_batch, re_pred_tail_batch)
